...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (aboutDesc && portfolioData.about) {
    ...
    if (aboutHighlights && portfolioData.about.certifications) {
      aboutHighlights.innerHTML = portfolioData.about.certifications
        .map(
          (cert) => `
                    <div class="glass-card p-4 text-center">
                        <div class="text-2xl mb-2">🎓</div>
                        <h4 class="font-bold text-sm mb-1">${cert.name}</h4>
                        ${cert.issuer ? `<p class="text-xs text-neon-teal mb-1">${cert.issuer}</p>` : ""}
                        <p class="text-xs text-gray-400">${cert.year}${cert.duration ? ` (${cert.duration})` : ""}</p>
                    </div>
                `
        )
        .join("");
  ...
  if (contactInfo && portfolioData.contact) {
    contactInfo.innerHTML = `
            <div class="glass-card p-6 inline-block">
                ...
                ${portfolioData.contact.phone ? `<p class="text-lg mb-2"><strong>Телефон:</strong> <a href="tel:${portfolioData.contact.phone}" class="text-neon-teal hover:underline">${portfolioData.contact.phone}</a></p>` : ""}
  ...
  if (projectsGrid && portfolioData.projects) {
    projectsGrid.innerHTML = portfolioData.projects
      .map(
        (project) => `
                <div class="glass-card p-6 hover:scale-105 transition-transform relative">
                    ${project.badge ? `<div class="absolute top-4 right-4 bg-gradient-to-r from-pink-500 to-purple-500 text-white px-3 py-1 rounded-full text-xs font-bold">${project.badge}</div>` : ""}
                    <h3 class="text-xl font-bold mb-2">${project.name}</h3>
                    ${project.status ? `<p class="text-sm text-yellow-400 mb-2">Статус: ${project.status}</p>` : ""}
